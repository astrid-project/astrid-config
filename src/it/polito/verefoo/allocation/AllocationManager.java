...
import it.polito.verefoo.jaxb.Filteringlink;
...
import it.polito.verigraph.functions.FilteringLinkFunction;
	...
	public void instantiateFunctions() {
		...
		nodes.values().forEach(allocationNode -> {
			...
			if(node.getFunctionalType() != null) {
				...
				}else if(node.getFunctionalType() == FunctionalTypes.FILTERINGLINK) {
					FilteringLinkFunction fl = new FilteringLinkFunction(allocationNode, ctx, nctx);
					allocationNode.setPlacedNF(fl);
					allocationNode.setTypeNF(FunctionalTypes.FILTERINGLINK);
				}
			}
	...
	public void configureFunctions() {
		...
		nodes.values().forEach(allocationNode -> { 
			...
			if(type.equals(FunctionalTypes.WEBCLIENT)) {
				...
				FilteringLinkFunction fl = (FilteringLinkFunction) no;
				fl.filteringLinkSendRules();
			}
		});
